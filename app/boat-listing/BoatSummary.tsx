import { postNewBoats } from '@/apis/ListingService';
...
const Page = () => {
    const { boatCategory, type, attributes } = useLocalSearchParams();
    const parseMotor = JSON.parse(attributes as any);
    ...
    const addtionalList = [
        {
            label: "Age",
            value: parseMotor?.age
        },
        {
            label: "Usage",
            value: parseMotor?.usage
        },
        {
            label: "Condition",
            value: parseMotor?.condition
        },
        {
            label: "Length",
            value: parseMotor?.length
        },
        {
            label: "Seller Type",
            value: parseMotor?.sellertype
    ...
    const onSubmit = async () => {
        ...
        try {
            ...
            formData.append("type_1", boatCategory as string);
            formData.append("type_2", type as string);
            formData.append("usage", parseMotor?.usage);
            formData.append("warranty", parseMotor?.warranty);
            formData.append("age", parseMotor?.age);
            formData.append("seller_type", parseMotor?.sellertype);
            formData.append("length", parseMotor?.length);
            formData.append("condition", parseMotor?.condition);
